...
window.onload = function() {
    ...
    // Gui buttons
    var buttons = [ { x: 30, y: 240, width: 200, height: 50, text: "Новая игра"},
                    { x: 30, y: 300, width: 200, height: 50, text: "Показать ход"},
                    { x: 30, y: 360, width: 200, height: 50, text: "Включить бот"}];
    ...
    function render() {
        ...
        drawCenterText("Счет:", 50, level.y+40, 150);
        drawCenterText(score, 50, level.y+70, 150);
        ...
        if (gameover) {
            ...
            context.font = "24px Verdana";
            drawCenterText("Игра закончена", level.x, level.y + levelheight / 2 + 10, levelwidth);
        }
    }
        ...
        for (var i=0; i<buttons.length; i++) {
            if (pos.x >= buttons[i].x && pos.x < buttons[i].x+buttons[i].width &&
                ...
                if (i == 0) {
                    ...
                    // Show Moves
                    showmoves = !showmoves;
                    buttons[i].text = (showmoves?"Непоказывать":"Показать") + " ход";
                } else if (i == 2) {
                    // AI Bot
                    aibot = !aibot;
                    buttons[i].text = (aibot?"Выключить":"Включить") + " AI бот";
                }
            }
        }
    }
